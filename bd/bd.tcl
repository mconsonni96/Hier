...
proc update_contents {cell_name args} {
	...
	set _VLNV(NI) "DigiLAB:ip:BeltBus_NodeInserter:2.4"
	set _VLNV(OC) "DigiLAB:ip:AXI4Stream_OverflowCounter:2.1"
	# --------------------------------------------------------------------------
	...
	set BIT_COARSE_CEC [get_property CONFIG.BIT_COARSE $cell_obj]
	...
	set AXI4_LENGTH [get_property CONFIG.AXI4_LENGTH $cell_obj]
     	...
     	#AXI4Stream_OverflowCounter generics
     	set BIT_FID [get_property CONFIG.BIT_FID $cell_obj]
     	set BIT_COARSE_CEC [get_property CONFIG.BIT_COARSE_CEC $cell_obj]
     	set BIT_COARSE [get_property CONFIG.BIT_COARSE $cell_obj]
     	set BIT_RESOLUTION [get_property CONFIG.BIT_RESOLUTION $cell_obj]
	...
	if {[get_bd_pins $cell_obj/CoarseCounter_CTD] == "" && ($CEC_VS_CTD_COUNTER == "CTD")} {
		create_bd_pin -dir I -from 3 -to 0 -type data CoarseCounter_CTD
	}
	...
	# ------- AXI4Stream_IperDecoder ---------
	# Create instance: AXI4Stream_IperDecoder_0, and set properties, BIT_COARSE = 1, where we use this bit to propagate the polarity (1 bit)
	# The coarse is creatded by the CEC so we had in pipeline also the coarse. Now it is better to call it BIT_PIPELINE, and BIT_TDL = BIT_SMP because of the decimation
	...
	set_property -dict [ list \
		...
		CONFIG.BIT_COARSE "$BIT_COARSE_CEC" \
	...
	# Create instance: AXI4Stream_OverflowCounter_0, and set properties
  	...
  	set_property -dict [ list \
		CONFIG.BIT_FID "$BIT_FID" \
		CONFIG.BIT_COARSE_CEC "$BIT_COARSE_CEC" \
